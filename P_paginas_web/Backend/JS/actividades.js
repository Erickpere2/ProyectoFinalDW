...
module.exports = {
    ...
    getActividadesAlumno: async (req, res, next, validationResult) => {
        try {
            var actividades = await alumActivity.find({id_estudiante:req.query.id});
    ...
    getDatosTarea: async (req, res, next, validationResult) => {
        try {
            ...
            var actividades = await activities.findOne({_id:req.query.id});
            res.status(200).send(actividades);
        } catch (err) {
            console.log(err);
